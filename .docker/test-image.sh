...
docker run --rm ${imageName} goxr --version 2>&1          | grep " Revision:   TEST${TRAVIS_COMMIT}TEST"
docker run --rm ${imageName} goxr-server --version 2>&1   | grep " Version:    TEST${TRAVIS_BRANCH}TEST"
docker run --rm ${imageName} goxr-server --version 2>&1   | grep " Revision:   TEST${TRAVIS_COMMIT}TEST"
